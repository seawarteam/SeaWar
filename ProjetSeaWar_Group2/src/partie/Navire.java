...
public class Navire extends Observable implements Serializable {	
    ...
    public Navire(String n, int vie, int depM, String nameJ, Orientation ori, Position posi, Observer obs) {
    	...
    	bloque = false;
    	//id = new ID_Navire(ID_Navire.getNewID());
    ...
    /**
     * Mets a  jour les variables d'instances lors d'un deplacement
     * @param pos : la case ou l'on veut aller 
    ...
    /**
     * @param degats a retirer aux pv du navire
     */
    ...
    **/
    ...
    /**
     * retourne toutes les cases visable par le canon primaire en prenant en compte les differantes possibilité de déplacement
     * rocher : les positions des cases rochers
     * obstacle = rocher + position des navires
     */
    ...
    /**
     * retourne toutes les cases visable par le canon secondaire en prenant en compte les differantes possibilité de déplacement
     * rocher : les positions des cases rochers
     * obstacle = rocher + position des navires
     */
...
//    	Ajout des cases que l'on peux toucher sans se deplacer
    ...
    public Set<Orientation> findOrientationsPossiblesForIA(Position pos, Set<Position> obstacle) {
    	Set<Vector<Object>> setVectPos = rechercheCasesAccessible(obstacle).get(pos);
    	Set<Orientation> resultat = new HashSet<Orientation>();
    	
    	if(bloque) {
    		resultat.add(Orientation.getDirOppose(dir));
    		return resultat;
    	}
    ...
    public int getPathLengh(Position pos, Orientation dir, Set<Position> obstacle) {
    	...
    	if(bloque) {
    		aDejaTire = true;
    		//aEteDeplace = true;
    		return this.dep;//TODO: new => tester !
    ...
    /**
     * Construction par recursion des cases Accessibles
     * @param map
     * @param deplace : nbre de deplacement encore possible
     * @param fileDattente : dernieres cases atteintes
     */
    			...
    			for(Vector<Object> vectVoisinRela : voisinsRela){
    				...
    				if(cellVoisinReel != null && (!obstacle.contains(cellVoisinReel) || cellVoisinReel.equals(this.pos))){
    					...
    					Vector <Object> vectInfo = new Vector<Object>(3);
							...
							vectInfo.add(2, vect);
    					// Ajouter les coordonnees dans la map
	...
	public boolean haveCanon(ID_Canon id) {
		if(canonP.getID() == id) {
			return true; 
		}
		if(canonS.getID() == id) {
	...
	public void setID(ID_Navire id_Navire) {
		id = id_Navire;		
	}
	
	public void updatePos() {
		pos = Position.getPosition(pos.getX(), pos.getY());
	}
